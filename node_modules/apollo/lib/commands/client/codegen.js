...
const watcher_1 = require("@atom/watcher");
...
class Generate extends Command_1.ClientCommand {
    async run() {
        ...
        if (watch) {
            await run().catch(() => { });
            const watcher = await watcher_1.watchPath(this.project.rootURI.fsPath, {}, events => {
                for (const event of events) {
                    if (!this.project.includesFile(event.path))
                        return;
                    this.project.fileDidChange(vscode_uri_1.default.file(event.path).toString());
                }
            });
